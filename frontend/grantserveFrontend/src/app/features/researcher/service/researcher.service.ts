...
export class ResearcherService {
  ...
  // responseType: 'blob' is critical for binary data like PDFs
...
getApplicationCount(researcherId: number): Observable<any> {
  return this.http.get<any>(`http://localhost:8081/application-service/GrantApplication/UserApplicationCount/${researcherId}`);
}

//compliance status 
// researcher.service.ts

// 1. Fetch the Officer's "To-Do" list
getPendingQueue(): Observable<ResearcherDocument[]> {
  return this.http.get<ResearcherDocument[]>(`${this.docUrl}/pending`);
}

// 2. The decision endpoint added recently. 
updateVerificationStatus(id: number, status: string): Observable<string> {
  return this.http.put(`${this.docUrl}/${id}/status`, {}, {
