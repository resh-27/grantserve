...
export class ProgramDataService {
  ...
  getBudgetsByProgramIds(programIds: number[]): Observable<any[]> {
    return this.http.get<any[]>(`${this.budgetUrl}/program`, { params: { programIds: programIds.join(',') } });
  }

  // Target: BudgetController -> getBudgetById()
  getBudgetByBudgetId(id: number): Observable<any> {
    return this.http.get<any>(`${this.budgetUrl}/${id}`);
  }
