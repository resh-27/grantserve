...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, ManagerHeaderComponent, ManagerAppCard],
  ...
  confirmInitiate(): void {
    ...
    this.service.initiateDisbursement({
      ...
      error: (err) => { 
        ...
        this.toast.show('Failed to initiate disbursement', 'danger');
  ...
  confirmPayment(): void {
    ...
    this.service.processPayment({
      ...
      error: (err) => { 
        ...
        this.toast.show('Failed to process payment', 'danger');
        this.isSubmitting = false;
      }
    });
  }
